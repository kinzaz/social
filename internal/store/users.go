...
func (s *UserStore) GetByID(ctx context.Context, id int64) (*User, error) {
	query := `
		SELECT users.id, users.username, users.email, users.password, users.created_at, roles.*
		FROM users JOIN roles ON (users.role_id = role.id) 
		WHERE users.id = $1 AND users.is_activate = true
	`
...
func (s *UserStore) GetByEmail(ctx context.Context, email string) (*User, error) {
	query := `
		...
		FROM users WHERE email = $1 AND is_activate = true
	`
...
func (s *UserStore) delete(ctx context.Context, tx *sql.Tx, id int64) error {
	query := `DELETE FROM users WHERE i = $1`
